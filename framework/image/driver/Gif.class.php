...
Class Gif {
	 ...
	 * 
	 * @access public
	 * @param string $content 待写入的当前帧数据流.
	 * @return resource
	 */
	public function frame($content = NULL) {
		if(NULL === $content) {
			$result = current($this->_frames);
			return FALSE === $result ? reset($this->_frames) : $result;
		} else {
			$this->_frames[key($this->_frames)] = $content;
		}
	}
	...
	public function save($destPath) {
		...
		return (boolean)file_put_contents($destPath, $gifEncoder->GetAnimation());
	}
